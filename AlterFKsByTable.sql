...
SET @TableName = 'TRADER'
	...
	(
	FKName NVARCHAR(255),
	ParentSchema NVARCHAR(255),
	ParentTable NVARCHAR(255), 
	ParentColumn NVARCHAR(255),
	RefSchema NVARCHAR(255),
	RefTable NVARCHAR(255),
	RefColumn NVARCHAR(255),
	is_disabled INT, 
	RefTableColumnID INT
	)

	INSERT INTO @Table1 (FKName,ParentSchema,ParentTable,ParentColumn,RefSchema,RefTable,RefColumn,is_disabled,RefTableColumnID)
	...
	JOIN sys.objects o2 on o2.object_id = fk.referenced_object_id
	JOIN sys.schemas s2 on s2.schema_id = o2.schema_id
	JOIN sys.foreign_key_columns fkc ON fkc.parent_object_id = fk.parent_object_id AND fkc.referenced_object_id = fk.referenced_object_id
	JOIN [INFORMATION_SCHEMA].[CONSTRAINT_COLUMN_USAGE] ccu ON  ccu.TABLE_NAME = o.name AND ccu.CONSTRAINT_NAME = fk.name
	JOIN sys.columns c ON  c.column_id = fkc.parent_column_id AND c.object_id = o.object_id AND c.column_id = fkc.parent_column_id AND c.name  = ccu.COLUMN_NAME
	JOIN sys.columns c2 ON  c2.column_id = fkc.referenced_column_id AND c2.object_id = o2.object_id AND c2.column_id = fkc.referenced_column_id
	WHERE s2.name = @SchemaName
	AND o2.name = @TableName
	ORDER BY 1,2,RefTableColumnID
  
--        DECLARE cFK CURSOR FOR
	;WITH cte (FKName,ParentSchema,ParentTable,ParentColumn,RefSchema,RefTable,RefColumn,is_disabled)
	AS (
		SELECT 
			FKName,
			ParentSchema,
...
SELECT 
	N'ALTER TABLE ' + ParentSchema + '.' + ParentTable + N' DROP CONSTRAINT [' + FKName + '];' AS DropFK,
	CASE WHEN Is_Disabled = 1
		THEN N'ALTER TABLE ' + ParentSchema + '.' + ParentTable + N' WITH NOCHECK ADD CONSTRAINT [' + FKName + '] FOREIGN KEY (' + ParentColumn + ') REFERENCES [' + RefSchema + '].[' + RefTable + '] (' + RefColumn + ')'
		ELSE N'ALTER TABLE ' + ParentSchema + '.' + ParentTable + N' WITH CHECK ADD CONSTRAINT [' + FKName + '] FOREIGN KEY (' + ParentColumn + ') REFERENCES [' + RefSchema + '].[' + RefTable + '] (' + RefColumn + ')' END AS AlterFK,
	CASE WHEN Is_Disabled = 1 
		THEN ' ALTER TABLE ' + ParentSchema + '.' + ParentTable + N' NOCHECK CONSTRAINT [' + FKName + '];'
		ELSE 'ALTER TABLE ' + ParentSchema + '.' + ParentTable + N' CHECK CONSTRAINT [' + FKName + '];' END AS AlterTable
FROM cte
